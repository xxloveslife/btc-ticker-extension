...
3. **点击弹窗** — 大字价格、**可交互蜡烛图**(TradingView `lightweight-charts`:滚轮缩放、拖动平移、拖价格轴调高低、十字光标;`5m/15m/1h/4h/1d` 切换,默认 5m;叠加 EMA20 线(随周期自动重算);图右上角显示当前K线收盘倒计时)、资金费率/结算倒计时/24h 高低。弹窗右上角的 `WS实时 / 轮询5s` 标识显示数据来源。
